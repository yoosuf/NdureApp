...
class WorkoutLog(models.Model):
    customer = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE)
    trainer = models.ForeignKey(settings.AUTH_USER_MODEL, related_name='trainer_logs', on_delete=models.SET_NULL, null=True, blank=True)
    workout_types = models.ManyToManyField(WorkoutType)
    date = models.DateField()
    duration = models.DurationField()
    notes = models.TextField(blank=True)
    user_package = models.ForeignKey(UserPackage, on_delete=models.SET_NULL, null=True, blank=True)
